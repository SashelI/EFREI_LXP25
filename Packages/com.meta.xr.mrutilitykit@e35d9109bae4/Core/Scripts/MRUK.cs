...
namespace Meta.XR.MRUtilityKit
{
    ...
    public partial class MRUK : MonoBehaviour
    {
        ...
        internal CameraRig _cameraRig { get; private set; } = new();
        public class CameraRig
        {
	        public MRUK mruk;
	        internal Transform trackingSpace => mruk.trackingSpace;
        }
        ...
        public MRUKRoom GetCurrentRoom()
        {
            ...
            {
                if (Camera.main.transform.position is Vector3 eyePos)
                {
            ...
            foreach (var roomAnchor in rooms)
            {
                ...
                foreach (var child in childAnchors)
                {
                    ...
                    if (child.TryGetComponent(out OVRLocatable locatable) && locatable.IsEnabled)
                    {
                        ...
                        {
                            var position = pose.ComputeWorldPosition(trackingSpace);
                            var rotation = pose.ComputeWorldRotation(trackingSpace);
