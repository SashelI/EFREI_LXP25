...
Shader "Astrolabe/Non-Canvas/Frontplate" {
...
CBUFFER_END
    ...
    void Scale_RGB_B1249(
        ...
        out half4 Result    )
    {
        Result = float4(Color.x,Color.y,Color.z,Scalar);
    }
    //BLOCK_END Scale_RGB
...
half4 frag(VertexOutput fragInput) : SV_Target
{
    ...
    // Mix_Colors (#1247) - Interpolate towards transparent
    half4 Transparent_Color = half4(Result_Q1249.rgb, 0); // Target is transparent (alpha = 0)
    half4 Color_At_T_Q1247 = lerp(Result_Q1249, Transparent_Color, float4(Is_Quad_Q1252, Is_Quad_Q1252, Is_Quad_Q1252, Is_Quad_Q1252));
