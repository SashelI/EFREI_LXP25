...
namespace Assets.Astrolabe.Scripts.MRTK_Enhanced
{
    ...
    public class AstrolabeRiggedHandMeshVisualizer : MonoBehaviour
    {
        [SerializeField]
        ...
        public bool UpdateJointsWhenNotRendered = false;
        ...
        private void Update()
        {
	        bool hasEntireHand = handsSubsystem.TryGetEntireHand(handNode, out IReadOnlyList<HandJointPose> joints);

			// Query all joints in the hand.
			if (!ShouldRenderHand() || !hasEntireHand)
            {
                ...
                if (!UpdateJointsWhenNotRendered)
                {
	                return;
				}
            }
                ...
                if (jointTransform != null)
                {
                    switch ((TrackedHandJoint)i)
                    {
                        ...
                        case TrackedHandJoint.LittleMetacarpal:
                            ...
                            var lookRotationVector = jointPose.Position - joints[(int)TrackedHandJoint.Wrist].Position;
                            if (lookRotationVector != Vector3.zero)
                            {
	                            jointTransform.rotation = Quaternion.LookRotation(lookRotationVector, jointPose.Up);
							}
                            break;
                        default:
	                        var defaultLookRotationVector = jointPose.Position - jointTransform.position;
	                        if (defaultLookRotationVector != Vector3.zero)
	                        {
		                        // For all other bones, do a simple "IK" from the rigged joint to the joint data's position.
		                        jointTransform.rotation =
			                        Quaternion.LookRotation(defaultLookRotationVector, joints[i - 1].Up);
	                        }
	                        break;
                    }
                }
        ...
        public bool TryGetHandJoint(TrackedHandJoint joint, out Pose pose)
        {
	        if (riggedVisualJointsArray != null && riggedVisualJointsArray.Length > (int)joint)
	        {
		        var jointTransform = riggedVisualJointsArray[(int)joint];

		        if (jointTransform != null)
		        {
			        pose = new Pose(jointTransform.position, jointTransform.rotation);
			        return true;
				}
	        }
