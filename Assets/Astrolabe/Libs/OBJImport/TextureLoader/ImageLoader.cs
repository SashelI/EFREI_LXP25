		...
		public static Texture2D LoadTexture(string fn)
		{
			...
			switch (ext)
			{
				...
				case ".crn":
					...
					{
						Log.WriteLine(
							"Could not load crunched texture " + name + " because its format is not supported (" +
							crnFormatByte + "): " + fn, LogMessageType.Error);
						break;
					}
				...
				default:
					Log.WriteLine("Could not load texture " + name + " because its format is not supported : " + fn,
